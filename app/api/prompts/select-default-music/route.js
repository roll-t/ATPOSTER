...
export async function POST(req) {
  try {
    ...
    // nhưng sẽ không tìm thấy file tương ứng trong public/audio/bg-music/default và báo nhầm lỗi 404.
    if (trackId && trackId.startsWith('lib_')) {
      ...
      const librarySource = path.join(process.cwd(), 'public', 'audio', 'bg-music', 'custom', libItem.filename);
    ...
    const defaultMusicSource = path.join(process.cwd(), 'public', 'audio', 'bg-music', 'default', `${trackId}.mp3`);
