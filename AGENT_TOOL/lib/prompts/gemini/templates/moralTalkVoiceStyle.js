...
1. Open with a reflective, rhetorical QUESTION addressed directly to "bạn" that draws the listener into the emotional premise of the topic — never open with a plain flat statement.
...
"""
Techniques to reproduce from this reference, applied to whatever topic is given:
1. Open with ONE short, blunt hook line that frames the whole list as hard-won truths learned through real-life experience, not textbook advice — and states the exact count of points, leading with the number itself, e.g. "10 quy tắc ngầm trong xã hội mà ai cũng nên biết." NEVER start this line with "Có" (e.g. NOT "Có 10 quy tắc..." — start straight with the number instead). That number MUST equal exactly how many list-point slides/lines follow, so the viewer knows upfront how long the list is and can follow along.
