...
import { syncYoutubeAvatar, getYoutubeProfileDir } from '@/lib/poster.js';
import fs from 'fs';
...
export async function DELETE(request, { params }) {
  try {
    ...
    // Xóa hồ sơ trình duyệt lâu dài
    const profileDir = getYoutubeProfileDir(id);
    if (fs.existsSync(profileDir)) {
      try {
        fs.rmSync(profileDir, { recursive: true, force: true });
      } catch (err) {
        console.error(`Không thể xóa hồ sơ trình duyệt ${profileDir}:`, err);
      }
    }
