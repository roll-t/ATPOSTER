...
const sourceFilesList = scanDirs.flatMap((dir) => sourceFiles(path.join(root, dir)));
const sourceCount = sourceFilesList.length;
const oversized = sourceFilesList
  .map((filePath) => ({ filePath, lines: readFileSync(filePath, 'utf8').split('\n').length }))
  .filter(({ lines }) => lines > 800)
  .sort((a, b) => b.lines - a.lines);

const legacyStudioImports = sourceFilesList
...
const domainViolations = sourceFiles(path.join(root, 'src/domain'))
  .filter((filePath) => forbiddenInnerDependency.test(readFileSync(filePath, 'utf8')))
  .map(relative);
const applicationViolations = sourceFiles(path.join(root, 'src/application'))
