...
import { readDb, writeDb, getSessionsDir, getUploadsDir, getProfilesDir, logDiagnosticError } from './db.js';

const execPromise = util.promisify(exec);

// ==========================================================================
// Theo dõi các trình duyệt/context đang chạy ngầm cho từng bài đăng, để có thể
// HỦY thật sự (đóng trình duyệt) khi người dùng bấm "Hủy" trên bài đang xử lý,
// thay vì chỉ đổi trạng thái trong DB trong khi tiến trình vẫn chạy ngầm.
// ==========================================================================
global.activeUploadHandles = global.activeUploadHandles || new Map();
global.cancelledPostIds = global.cancelledPostIds || new Set();

function registerUploadHandle(postId, handle) {
  global.activeUploadHandles.set(postId, handle);
}

function unregisterUploadHandle(postId) {
  global.activeUploadHandles.delete(postId);
}

/**
 * Hủy tiến trình đăng bài đang chạy ngầm cho một postId (nếu có trình duyệt đang mở).
 * @returns {Promise<boolean>} true nếu tìm thấy và đã yêu cầu đóng trình duyệt đang chạy.
 */
export async function cancelUpload(postId) {
  const handle = global.activeUploadHandles.get(postId);
  if (!handle) return false;

  try {
    await handle.close();
  } catch (e) {
    // Trình duyệt có thể đã đóng sẵn hoặc mất kết nối, bỏ qua lỗi.
  }
  unregisterUploadHandle(postId);
  return true;
}
...
// ==========================================================================
// Hồ sơ trình duyệt lưu trữ lâu dài (persistent profile) cho tài khoản YouTube.
// Trước đây hệ thống dùng chế độ InPrivate/Incognito + chỉ nạp lại cookie mỗi
// lần chạy, khiến Google luôn coi đây là "thiết bị lạ" và liên tục yêu cầu
// xác minh lại / kick phiên đăng nhập. Dùng persistent context giữ nguyên
// toàn bộ dấu vết thiết bị (cookie, cache, IndexedDB...) giữa các lần chạy
// nên chỉ cần đăng nhập 1 lần duy nhất.
// ==========================================================================

const SESSION_READY_MARKER = '.session-ready';

export function getYoutubeProfileDir(accountId) {
  return path.join(getProfilesDir(), `yt_${accountId}`);
}

export function hasYoutubeSession(accountId) {
  return fs.existsSync(path.join(getYoutubeProfileDir(accountId), SESSION_READY_MARKER));
}

function markYoutubeSessionReady(accountId) {
  const profileDir = getYoutubeProfileDir(accountId);
  if (!fs.existsSync(profileDir)) {
    fs.mkdirSync(profileDir, { recursive: true });
  }
  fs.writeFileSync(path.join(profileDir, SESSION_READY_MARKER), new Date().toISOString());
}

/**
 * Khởi chạy hồ sơ trình duyệt lâu dài cho một tài khoản YouTube (thay cho
 * InPrivate/Incognito dùng 1 lần). Nếu tài khoản có session cũ dạng cookie
 * (data/sessions/{accountId}.json) và chưa từng có hồ sơ lâu dài, tự động
 * nâng cấp bằng cách nạp cookie đó vào hồ sơ mới trong lần chạy đầu tiên -
 * không cần đăng nhập lại thủ công.
 */
async function launchYoutubePersistentContext(accountId, { headless = false } = {}) {
  const userDataDir = getYoutubeProfileDir(accountId);
  if (!fs.existsSync(userDataDir)) {
    fs.mkdirSync(userDataDir, { recursive: true });
  }

  const legacySessionPath = path.join(getSessionsDir(), `${accountId}.json`);
  const shouldSeedLegacySession = !hasYoutubeSession(accountId) && fs.existsSync(legacySessionPath);
  ...
  if (shouldSeedLegacySession) {
    baseOptions.storageState = legacySessionPath;
    console.log(`[YouTube ${accountId}] Đang nâng cấp session cũ sang hồ sơ trình duyệt lâu dài...`);
  }

  let context;
  try {
    context = await chromium.launchPersistentContext(userDataDir, { ...baseOptions, channel: 'msedge' });
  } catch (errEdge) {
    try {
      context = await chromium.launchPersistentContext(userDataDir, { ...baseOptions, channel: 'chrome' });
    } catch (errChrome) {
...
export async function loginYoutubeAccount(accountId, email = '', password = '') {
  console.log(`[Login YouTube ${accountId}] Đang khởi chạy hồ sơ trình duyệt lâu dài...`);
  const context = await launchYoutubePersistentContext(accountId, { headless: false });
  const page = await context.newPage();
      ...
      try {
        ...
        if (url.includes('studio.youtube.com') && !url.includes('accounts.google.com') && !url.includes('signin')) {
          ...
          if (!page.isClosed()) {
            ...
            // Đánh dấu hồ sơ trình duyệt lâu dài đã sẵn sàng (persistent context tự lưu cookie/cache vào đĩa)
            markYoutubeSessionReady(accountId);
            console.log(`[Login YouTube ${accountId}] Đã lưu hồ sơ trình duyệt lâu dài tại: ${getYoutubeProfileDir(accountId)}`);
          }
          break;
        }
      } catch (errLoop) {
...
export async function uploadVideo(post, sessionFile) {
  ...
  registerUploadHandle(post.id, browser);
  ...
  try {
    ...
    unregisterUploadHandle(post.id);
    await browser.close();
  }
}
...
export async function runUploadInBackground(postId) {
  ...
  try {
    ...
    // Tiến hành upscale lên 2K nếu có yêu cầu
    if (post.upscale2k) {
      try {
        await upscaleVideoTo2K(post.videoFilename, post.upscaleResolution || '1440:2560');
      } catch (upscaleErr) {
        console.error(`[Background Upscale Error] Lỗi upscale video (tiếp tục sử dụng bản gốc):`, upscaleErr.message);
      }
    }
    ...
    if (account.videoType === 'tiktok') {
      ...
      if (account.type === 'adspower') {
        ...
        videoUrl = await uploadVideoYoutube(post);
      }
    }
    ...
    const wasCancelled = global.cancelledPostIds.has(postId);
    
    if (!wasCancelled) {
      logDiagnosticError(
        'Đăng bài thất bại (Background Upload)',
        error.message || 'Lỗi không xác định khi đăng bài.',
        error.stack || '',
        'lib/poster.js',
        { postId, accountId: post.accountId, platform: post.platform }
      ).catch(() => {});
    }
    ...
    if (activePost) {
      ...
      activePost.error = wasCancelled ? 'Đã hủy thủ công bởi người dùng.' : (error.message || 'Lỗi không xác định.');
      await writeDb(currentDb);
    }
    global.cancelledPostIds.delete(postId);
  }
}
...
async function automateYoutubeUploadInner(page, post) {
  ...
  if (currentUrl.includes('accounts.google.com') || currentUrl.includes('signin') || currentUrl.includes('/login')) {
    ...
    // Đánh dấu hồ sơ trình duyệt lâu dài đã sẵn sàng (persistent context tự lưu cookie/cache vào đĩa)
    try {
      markYoutubeSessionReady(post.accountId);
      console.log(`[YouTube Upload ${post.id}] Đã cập nhật hồ sơ đăng nhập lâu dài cho tài khoản.`);
    } catch (errSave) {
      console.error(`[YouTube Upload ${post.id}] Không đánh dấu được hồ sơ đăng nhập:`, errSave.message);
    }
  ...
  // Điền tiêu đề (YouTube giới hạn tối đa 100 kí tự và cấm ký tự '<', '>')
  let cleanTitle = post.caption.replace(/[<>]/g, '');
  cleanTitle = cleanTitle.length > 100 ? cleanTitle.slice(0, 97) + '...' : cleanTitle;
  ...
  await page.keyboard.press('Backspace');
  await page.waitForTimeout(400 + Math.random() * 200); // Trễ ngẫu nhiên

  // Gõ caption mới bằng thuật toán giả lập tốc độ gõ của người thật
  await titleInput.click();
  await page.waitForTimeout(300 + Math.random() * 200);
  await typeLikeHuman(page, cleanTitle);
  ...
  // Chờ nút Next được kích hoạt hoàn toàn ở Bước 1
  console.log(`[YouTube Upload ${post.id}] Chờ nút Next sẵn sàng ở Bước 1...`);
  ...
  // Chờ nút Next được kích hoạt hoàn toàn ở Bước 2
  console.log(`[YouTube Upload ${post.id}] Chờ nút Next sẵn sàng ở Bước 2...`);
  await page.waitForFunction(() => {
    const btn = document.querySelector('#next-button');
    return btn && !btn.hasAttribute('disabled') && btn.getAttribute('aria-disabled') !== 'true';
  }, { timeout: 20000 });
  ...
  // Chờ nút Next được kích hoạt hoàn toàn ở Bước 3
  console.log(`[YouTube Upload ${post.id}] Chờ nút Next sẵn sàng ở Bước 3...`);
  await page.waitForFunction(() => {
    const btn = document.querySelector('#next-button');
    return btn && !btn.hasAttribute('disabled') && btn.getAttribute('aria-disabled') !== 'true';
  }, { timeout: 20000 });
  ...
  await publicRadio.waitFor({ state: 'visible', timeout: 25000 });
  await publicRadio.scrollIntoViewIfNeeded();
  await publicRadio.click();
  await randomWait(page, 1.2, 2.5);
  ...
  await randomWait(page, 8.0, 11.0);
...
/**
 * Đăng video lên YouTube Shorts sử dụng hồ sơ trình duyệt lâu dài đã đăng nhập.
 * @param {object} post - Đối tượng post từ DB
 */
export async function uploadVideoYoutube(post) {
  const accountId = post.accountId;

  if (!hasYoutubeSession(accountId)) {
    console.log(`[YouTube Upload ${post.id}] Chưa có hồ sơ đăng nhập lâu dài. Tự động kích hoạt trình duyệt để bạn đăng nhập...`);
    try {
      const db = await readDb();
      ...
      await loginYoutubeAccount(accountId, email, password);
    } catch (loginErr) {
      throw new Error(`Đăng nhập thất bại: ${loginErr.message}`);
    }

    if (!hasYoutubeSession(accountId)) {
      throw new Error(`Không tạo được hồ sơ đăng nhập sau khi đăng nhập thủ công.`);
    }
    console.log(`[YouTube Upload ${post.id}] Đăng nhập thành công! Tiếp tục tiến trình đăng video...`);
  }

  console.log(`[YouTube Upload ${post.id}] Khởi chạy hồ sơ trình duyệt lâu dài để đăng video...`);
  const context = await launchYoutubePersistentContext(accountId, { headless: false });
  registerUploadHandle(post.id, context);
...
export async function uploadVideoYoutubeAdsPower(post, profileId) {
  ...
  try {
    ...
    return await automateYoutubeUpload(page, post);
  } finally {
    unregisterUploadHandle(post.id);
    try {
      await browser.close();
...
export async function syncYoutubeAvatar(accountId, base64Image) {
  ...
  try {
    ...
    if (isAdsPower) {
      ...
      if (!hasYoutubeSession(accountId)) {
        console.log(`[Avatar Sync] Chưa có hồ sơ đăng nhập lâu dài cho ${accountId}`);
        return;
      }

      console.log(`[Avatar Sync] Khởi chạy hồ sơ trình duyệt lâu dài...`);
      context = await launchYoutubePersistentContext(accountId, { headless: true });
    }
...
/**
 * Kiểm tra xem session YouTube của tài khoản còn hoạt động hay không.
 * @param {string} accountId - ID của tài khoản
 * @returns {Promise<{status: 'active'|'expired'|'error', reason?: string}>}
 */
export async function checkYoutubeSession(accountId) {
  if (!hasYoutubeSession(accountId)) {
    return { status: 'expired', reason: 'Chưa từng đăng nhập hoặc không có session.' };
  }

  console.log(`[Check YouTube ${accountId}] Khởi chạy trình duyệt ở chế độ headless để kiểm tra session...`);
  let context;
  try {
    context = await launchYoutubePersistentContext(accountId, { headless: true });
    const page = await context.newPage();
    ...
    // Đợi một trong các phần tử đặc trưng của Studio (đăng nhập thành công) hoặc Google Login (hết hạn)
    try {
      await Promise.race([
        page.waitForSelector('#channel-title, ytcp-navigation-drawer, #avatar-img, ytcp-app', { state: 'attached', timeout: 15000 }),
        page.waitForSelector('input[type="email"], #identifierId, [name="identifier"]', { state: 'attached', timeout: 15000 })
      ]);
    } catch (e) {
      console.log(`[Check YouTube ${accountId}] Hết hạn chờ đợi phần tử đặc trưng, tiến hành kiểm tra trực tiếp URL.`);
    }

    const url = page.url();
    console.log(`[Check YouTube ${accountId}] URL sau khi tải ổn định: ${url}`);
    
    if (url.includes('accounts.google.com') || url.includes('signin')) {
      return { status: 'expired', reason: 'Phiên đăng nhập hết hạn / Bị Google kick ra.' };
    }
    
    if (url.includes('studio.youtube.com') || url.includes('youtube.com/channel') || url.includes('youtube.com/@')) {
      return { status: 'active' };
    }
    
    // Kiểm tra xem trang có chứa phần tử đặc trưng của YouTube Studio hay không phòng trường hợp chuyển hướng URL chậm
    const isStudioVisible = await page.evaluate(() => {
      return !!document.querySelector('#channel-title') || !!document.querySelector('ytcp-navigation-drawer') || !!document.querySelector('ytcp-app');
    });
    ...
    return { status: 'expired', reason: 'Không thể truy cập studio.youtube.com (URL không hợp lệ)' };
  } catch (err) {
    console.error(`[Check YouTube ${accountId}] Lỗi trong quá trình kiểm tra:`, err);
    return { status: 'error', reason: err.message || 'Lỗi kiểm tra trình duyệt.' };
  } finally {
    if (context) {
      try {
        await context.close();
...
// Thuật toán giả lập tốc độ gõ phím của con người kèm sai lệch ngẫu nhiên

async function typeLikeHuman(page, text) {
  for (const char of text) {
    await page.keyboard.type(char);
    // Độ trễ ngẫu nhiên giữa các lần gõ (dao động từ 40ms đến 120ms)
    const delay = 40 + Math.random() * 80;
    await page.waitForTimeout(delay);
  }
}

// Helper sinh độ trễ ngẫu nhiên để đánh lạc hướng AI quét hành vi của YouTube
async function randomWait(page, min = 1, max = 3) {
  const ms = (min + Math.random() * (max - min)) * 1000;
  await page.waitForTimeout(ms);
}

export async function upscaleVideoTo2K(videoFilename, resolution = '1440:2560') {
  const uploadsDir = getUploadsDir();
  const inputPath = path.join(uploadsDir, videoFilename);
  const tempOutputFilename = `upscaled_${Date.now()}_${videoFilename}`;
  const outputPath = path.join(uploadsDir, tempOutputFilename);
  
  const ffmpegPath = path.join(path.resolve('data'), 'ffmpeg.exe');
  
  if (!fs.existsSync(inputPath)) {
    throw new Error(`Không tìm thấy file video đầu vào để upscale: ${inputPath}`);
  }
  if (!fs.existsSync(ffmpegPath)) {
    throw new Error(`Chưa cài đặt FFmpeg. Không thể thực hiện upscale.`);
  }
  
  console.log(`[FFmpeg Upscale] Đang nâng độ phân giải video ${videoFilename} lên ${resolution}...`);
  ...
  const cmd = `"${ffmpegPath}" -i "${inputPath}" -vf "${filter}" -c:v libx264 -crf 18 -preset superfast -c:a copy "${outputPath}"`;
  ...
  if (fs.existsSync(outputPath) && fs.statSync(outputPath).size > 0) {
    ...
    console.log(`[FFmpeg Upscale] Upscale video thành công!`);
  } else {
    throw new Error('File xuất ra bị rỗng hoặc lỗi FFmpeg.');
  }
}
