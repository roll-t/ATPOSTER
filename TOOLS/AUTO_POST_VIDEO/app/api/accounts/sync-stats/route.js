...
async function runBackgroundSync() {
  try {
    ...
    for (let i = 0; i < accounts.length; i++) {
      ...
      // Lưu lịch sử số liệu (tối đa 20 lần quét gần nhất) để vẽ biểu đồ xu hướng thật
      acc.statsHistory = Array.isArray(acc.statsHistory) ? acc.statsHistory : [];
      acc.statsHistory.push({
        date: acc.statsUpdatedAt,
        subscribers: stats.subscribers,
        views: stats.views,
        videoCount: stats.videoCount
      });
      if (acc.statsHistory.length > 20) {
        acc.statsHistory = acc.statsHistory.slice(-20);
      }
