...
export default function AccountsPage() {
  ...
  // Trạng thái modal Quản Lý Danh Mục
  const [showCategoryModal, setShowCategoryModal] = useState(false);

  // Trạng thái kiểm tra session
    ...
    try {
      ...
      if (res.ok && data.success) {
        setAccounts(prev => prev.map(acc => {
          if (acc.id === id) {
            return { ...acc, status: data.status, sessionError: data.reason || null };
          }
          return acc;
        }));
      } else {
        alert(data.error || 'Lỗi khi kiểm tra session.');
      }
    } catch (err) {
  ...
  const handleCheckAllSessions = async () => {
    const accountsToCheck = accounts.filter(acc => !checkingMap[acc.id]);
    if (accountsToCheck.length === 0) return;
    for (const acc of accountsToCheck) {
      await handleCheckSession(acc.id);
    }
  };

  // Trạng thái đăng nhập lại
  const [reloggingMap, setReloggingMap] = useState({});

  const handleRelogin = async (id) => {
    // Tự động copy email của tài khoản vào clipboard để tiện paste
    const targetAcc = accounts.find(acc => acc.id === id);
    if (targetAcc && targetAcc.email) {
      try {
        await navigator.clipboard.writeText(targetAcc.email);
        console.log(`[Relogin] Đã copy email vào clipboard: ${targetAcc.email}`);
      } catch (err) {
        console.warn('[Relogin] Không thể tự động copy email vào clipboard:', err);
      }
    }

    setReloggingMap(prev => ({ ...prev, [id]: 'running' }));
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.success) {
        alert(data.error || 'Không thể bắt đầu đăng nhập lại.');
        setReloggingMap(prev => {
          const next = { ...prev };
          delete next[id];
          return next;
        });
      ...
      // Polling kiểm tra trạng thái chạy ngầm
      const pollInterval = setInterval(async () => {
        try {
          const statusRes = await fetch(`/api/accounts?id=${id}`);
          if (!statusRes.ok) {
            clearInterval(pollInterval);
            alert('Không tìm thấy phiên đăng nhập lại.');
            setReloggingMap(prev => {
              const next = { ...prev };
              delete next[id];
              return next;
          ...
          if (statusData.status === 'success') {
            ...
            clearInterval(pollInterval);
            alert(statusData.error || 'Đăng nhập lại thất bại.');
            setReloggingMap(prev => {
              const next = { ...prev };
              delete next[id];
              return next;
          ...
          clearInterval(pollInterval);
          alert('Lỗi kết nối khi kiểm tra trạng thái đăng nhập lại.');
          setReloggingMap(prev => {
            const next = { ...prev };
            delete next[id];
            return next;
      ...
      alert('Đã xảy ra lỗi khi kết nối máy chủ để đăng nhập lại.');
      setReloggingMap(prev => {
        const next = { ...prev };
        delete next[id];
        return next;
  ...
  // Đóng modal Quản Lý Danh Mục bằng phím Esc
  useEffect(() => {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '30px', alignItems: 'start' }}>
        ...
        <div style={{ position: 'sticky', top: '20px', maxHeight: 'calc(100vh - 100px)', overflowY: 'auto', paddingRight: '4px' }}>
            ...
            <form onSubmit={handleAddAccount}>
              ...
              <div className="form-group" style={{ marginTop: '16px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px' }}>
                  <label className="form-label" style={{ marginBottom: 0 }}>Danh mục kênh</label>
                  <button
                    type="button"
                    onClick={() => setShowCategoryModal(true)}
                    style={{ background: 'none', border: 'none', color: 'var(--secondary)', fontSize: '0.75rem', fontWeight: 600, cursor: 'pointer', padding: 0, display: 'flex', alignItems: 'center', gap: '4px' }}
                  >
                    ⚙️ Quản lý danh mục
                  </button>
                </div>
        ...
        <div className="glass-card">
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px', flexWrap: 'wrap', gap: '10px' }}>
            <h3 style={{ fontSize: '1.25rem', fontWeight: 700 }}>Danh Sách Kênh Đã Liên Kết ({accounts.length})</h3>
            <div style={{ display: 'flex', gap: '10px' }}>
              <button
                type="button"
                onClick={handleCheckAllSessions}
                className="btn btn-secondary"
                disabled={accounts.length === 0 || Object.keys(checkingMap).length > 0}
                style={{
                  padding: '8px 14px',
                  fontSize: '0.8rem',
                  display: 'flex',
              ...
              >
                {Object.keys(checkingMap).length > 0 ? (
                  <>
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              ...
                              {reloggingMap[acc.id] === 'running' ? (
                                <span style={{
                                  ...
                                  <div style={{ width: '8px', height: '8px', border: '1px solid rgba(254, 44, 85, 0.2)', borderTopColor: 'var(--primary)', borderRadius: '50%', animation: 'spin 1.5s linear infinite' }}></div>
                                  Đang kết nối lại...
                                </span>
                              ) : checkingMap[acc.id] ? (
                                <span style={{
                                  ...
                                  <div style={{ width: '8px', height: '8px', border: '1px solid rgba(37, 244, 238, 0.2)', borderTopColor: 'var(--secondary)', borderRadius: '50%', animation: 'spin 1.5s linear infinite' }}></div>
                                  Đang check...
                                </span>
                              ) : acc.status === 'active' ? (
                                <span style={{
                                  ...
                                  fontWeight: 500,
                                  whiteSpace: 'nowrap'
                                }} title="Phiên đăng nhập đang hoạt động ổn định">
                                  ✓ Hoạt động
                                </span>
                              ) : acc.status === 'expired' ? (
                                <span style={{
                                  ...
                                  fontWeight: 500,
                                  whiteSpace: 'nowrap'
                                }} title={acc.sessionError || "Phiên đăng nhập đã hết hạn hoặc bị Google kick ra. Vui lòng đăng nhập lại."}>
                                  ✗ Hết hạn / Bị kick
                                </span>
                              ) : acc.status === 'error' ? (
                                <span style={{
                                  ...
                                  fontWeight: 500,
                                  whiteSpace: 'nowrap'
                                }} title={acc.sessionError || "Lỗi kiểm tra session"}>
                                  ! Lỗi session
                                </span>
                              ) : (
                                <span style={{
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                          ...
                          {reloggingMap[acc.id] === 'running' ? (
                            ...
                            >
                              <div style={{ width: '12px', height: '12px', border: '1.5px solid rgba(254, 44, 85, 0.2)', borderTopColor: 'var(--primary)', borderRadius: '50%', animation: 'spin 1s linear infinite' }}></div>
                              Đang kết nối...
                            </button>
                          ) : acc.status === 'expired' ? (
                            <button
                              ...
                              style={{
                                ...
                                color: '#fff',
                                boxShadow: '0 2px 8px rgba(254, 44, 85, 0.2)'
                              }}
                            >
                              Đăng nhập lại
                            </button>
                            ...
                            <button
                              ...
                              disabled={checkingMap[acc.id]}
                              className="btn btn-secondary"
                              style={{
                                padding: '8px 14px',
                                fontSize: '0.85rem',
                                border: checkingMap[acc.id] ? '1px solid rgba(255,255,255,0.1)' : '1px solid rgba(37, 244, 238, 0.15)',
                                color: checkingMap[acc.id] ? 'var(--text-muted)' : 'var(--secondary)'
                              }}
                            >
                              {checkingMap[acc.id] ? 'Đang check...' : 'Kiểm tra'}
                            </button>
                          )}
                          <button
      ...
      {showCategoryModal && (
        ...
        >
          <div
            ...
            style={{ width: '100%', maxWidth: '460px', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}
          >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3 style={{ fontSize: '1.2rem', fontWeight: 700 }}>Quản Lý Danh Mục</h3>
              <button
                type="button"
                onClick={() => { setShowCategoryModal(false); setEditingCategoryName(null); }}
                style={{ background: 'none', border: 'none', color: 'var(--text-muted)', fontSize: '1.3rem', lineHeight: 1, cursor: 'pointer', padding: '4px' }}
                aria-label="Đóng"
              >
                ×
              </button>
