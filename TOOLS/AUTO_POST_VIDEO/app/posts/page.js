...
export default function PostsPage() {
  ...
  const fetchData = async () => {
    try {
      ...
      // Bỏ qua log lỗi nếu là do request bị hủy khi reload trang hoặc chuyển hướng (Failed to fetch)
  ...
  // Nhờ hệ thống tự tính giờ đăng tối ưu cho kênh đã chọn (cân bằng giữa khung giờ vàng và an toàn chống spam)
  const handleAutoSchedule = async () => {
    if (!selectedAccountId) {
      setFormError('Vui lòng chọn tài khoản đăng trước khi dùng lên lịch tự động.');
      return;
    }
    ...
    try {
      ...
      if (res.ok && data.success) {
        ...
        setFormError(data.error || 'Không thể tính lịch đăng tự động.');
      }
    } catch (err) {
      setFormError('Lỗi kết nối khi tính lịch đăng tự động.');
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    // YouTube không hỗ trợ ảnh thu nhỏ tùy chỉnh cho Shorts nên không gửi lên nếu chọn loại này
    if (thumbnailFile && videoType !== 'shorts') {
  ...
  const handleCancelPost = async (postId) => {
    if (actionLoadingId) return;
    if (!confirm('Bạn có chắc chắn muốn HỦY bài đăng đang xử lý này? Trình duyệt đang đăng tải (nếu có) sẽ bị đóng lại và bài viết sẽ chuyển sang trạng thái Lỗi, bạn có thể đăng lại sau.')) {
      return;
    }

    setActionLoadingId(postId);
    try {
      const res = await fetch(`/api/posts/${postId}/cancel`, {
        method: 'POST'
      });
      if (res.ok) {
        fetchData();
        ...
        const data = await res.json();
        alert(data.error || 'Không thể hủy bài đăng.');
      }
    } catch (error) {
      alert('Lỗi kết nối khi hủy bài đăng.');
    } finally {
      setActionLoadingId(null);
    }
  };
          ...
          <form onSubmit={handleSubmit}>
            <div className="form-group">
              ...
              <select
                ...
                onChange={(e) => { setSelectedAccountId(e.target.value); setScheduleReason(''); }}
            ...
            <div className="form-group">
              ...
              <div style={{ display: 'flex', gap: '8px', background: 'rgba(255,255,255,0.03)', padding: '4px', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                <button
                  type="button"
                  onClick={() => { setVideoType('shorts'); removeThumbnail(); }}
                  style={{
            ...
            {/* YouTube không hỗ trợ ảnh thu nhỏ tùy chỉnh cho Shorts nên chỉ hiện với Video Thường */}
            {videoType === 'shorts' ? (
              <div className="form-group">
                <label className="form-label">Ảnh Thu Nhỏ / Thumbnail</label>
                <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', background: 'rgba(255,255,255,0.03)', border: '1px dashed rgba(255,255,255,0.1)', borderRadius: '8px', padding: '12px', lineHeight: 1.4 }}>
                  YouTube không hỗ trợ đặt ảnh thu nhỏ tùy chỉnh cho Shorts — hệ thống sẽ tự động dùng khung hình do YouTube chọn. Chuyển sang "YouTube Thường" ở trên nếu bạn cần đặt ảnh thu nhỏ riêng.
                </div>
              </div>
            ) : (
            <div className="form-group">
            ...
            <div className="form-group">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '8px', flexWrap: 'wrap', gap: '6px' }}>
                <label className="form-label" style={{ marginBottom: 0 }}>Hẹn giờ đăng (để trống nếu muốn ĐĂNG NGAY)</label>
                <button
                  type="button"
                  onClick={handleAutoSchedule}
                  disabled={isSubmitting || isRecommending || !selectedAccountId}
                  title={!selectedAccountId ? 'Chọn tài khoản đăng trước' : 'Để hệ thống tự tính giờ đăng tối ưu, cân bằng giữa khung giờ nhiều người xem và tránh dấu hiệu spam'}
                  style={{
                    background: 'none',
                    border: 'none',
                    color: 'var(--secondary)',
                    fontSize: '0.75rem',
                    fontWeight: 600,
                    cursor: (isSubmitting || isRecommending || !selectedAccountId) ? 'not-allowed' : 'pointer',
                    padding: 0,
                    opacity: (isSubmitting || isRecommending || !selectedAccountId) ? 0.5 : 1
                  }}
                >
                  {isRecommending ? '⏳ Đang tính giờ tối ưu...' : '🪄 Lên lịch tự động'}
                </button>
              </div>
              <input
                type="datetime-local"
                className="form-control"
                value={scheduledAt}
                onChange={(e) => { setScheduledAt(e.target.value); setScheduleReason(''); }}
                disabled={isSubmitting}
              />
              {scheduleReason && (
                <div style={{ fontSize: '0.72rem', color: 'var(--secondary)', marginTop: '8px', lineHeight: 1.4, background: 'rgba(37,244,238,0.06)', border: '1px solid rgba(37,244,238,0.15)', padding: '8px 10px', borderRadius: '6px' }}>
                  💡 {scheduleReason}
                </div>
              )}
            </div>
                ...
                <div key={post.id} style={{ padding: '16px', background: 'rgba(255, 255, 255, 0.02)', border: '1px solid rgba(255, 255, 255, 0.05)', borderRadius: '12px', display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    ...
                    <div style={{ display: 'flex', gap: '10px' }}>
                      ...
                      {(post.status === 'pending' || post.status === 'failed') && (
                        <button
                          ...
                          className="btn btn-secondary"
                          style={{ padding: '6px 12px', fontSize: '0.75rem', borderRadius: '6px', opacity: actionLoadingId === post.id ? 0.6 : 1 }}
                          disabled={actionLoadingId === post.id}
                        >
                          {actionLoadingId === post.id ? 'Đang xử lý...' : (post.status === 'failed' ? 'Đăng lại' : 'Đăng Ngay')}
                        </button>
                      )}
                      {post.status === 'processing' && (
                        <button
                          ...
                          style={{ padding: '6px 12px', fontSize: '0.75rem', borderRadius: '6px', background: 'rgba(255, 71, 87, 0.15)', color: 'var(--danger)', border: '1px solid rgba(255, 71, 87, 0.2)', opacity: actionLoadingId === post.id ? 0.6 : 1 }}
                          disabled={actionLoadingId === post.id}
                        >
                          {actionLoadingId === post.id ? 'Đang hủy...' : '⏹️ Hủy'}
                        </button>
                      )}
                      {post.status !== 'processing' && (
                        <button
                  ...
                  {post.status === 'processing' && (
                    <div style={{ fontSize: '0.8rem', color: 'var(--secondary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                      ...
                      {post.processingStartedAt && (
                        <span style={{ color: 'var(--text-muted)' }}>
                          (đã {Math.max(0, Math.floor((Date.now() - new Date(post.processingStartedAt).getTime()) / 60000))} phút — sẽ tự động hủy nếu vượt quá 20 phút)
                        </span>
                      )}
                    </div>
                  )}
