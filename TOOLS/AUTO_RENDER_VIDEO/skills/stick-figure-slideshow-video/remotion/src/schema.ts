...
export const sceneSchema = z.object({
  ...
  // the photo/illustration shown for this scene. Optional when `elements` is
  // provided (composed scene drawn directly in Remotion — no image file needed).
  ...
  // COMPOSED SCENE — danh sách asset elements để Remotion ghép trực tiếp trên nền trắng.
  // Khi có trường này thì KHÔNG cần image file — SceneCanvas thay thế SceneImage, toàn bộ
  // nhân vật/đạo cụ/ký hiệu đều được vẽ từ thư viện SVG sẵn có (xem src/assets/index.ts).
  // Mỗi element có id asset, vị trí (x,y) tính theo % canvas, và scale.
  elements: z.array(assetElementSchema).optional(),

  // HÉ LỘ DẦN TỪNG Ô CỦA MỘT ẢNH NHIỀU CHI TIẾT.
  //
  // Thay vì sinh mỗi câu nói một ảnh riêng (chậm, tốn, và dễ lệch nét vẽ giữa các ảnh), ta sinh
  // ĐÚNG 1 ảnh chứa sẵn toàn bộ chi tiết của cả đoạn, chia thành lưới ô đều nhau, rồi để Remotion
  // hé lộ thêm một ô theo mỗi câu. Người xem thấy chi tiết mới xuất hiện liên tục — đúng cảm giác
  // "cắt nhanh" của video whiteboard — mà toàn bộ đoạn chỉ tốn 1 lần sinh ảnh.
  //
  // Dùng chung với imageGroup (xem imageSlideshow.js): các segment cùng nhóm trỏ về cùng 1 ảnh,
  // segment thứ k trong nhóm đặt revealIndex = k.
  //
  // Cố ý dùng LƯỚI cố định chứ không phải toạ độ x/y tự do: Gemini viết prompt ảnh nhưng không hề
  // biết máy sinh ảnh sẽ đặt chi tiết ở đâu trong khung, nên toạ độ tự do gần như luôn trỏ trượt.
  // Còn lưới thì prompt ép được ("chia 3 ô dọc, ô trái: ..., ô giữa: ...") và cắt lại chính xác.
  // Tên đọc là "<số hàng>x<số cột>": "1x3" = 1 hàng 3 cột (chia NGANG, hợp video 16:9),
  // "3x1" = 3 hàng 1 cột (chia DỌC, hợp video 9:16 vì mỗi ô mới không bị bẹp dí).
  revealLayout: z.enum(["1x2", "1x3", "1x4", "2x1", "3x1", "4x1", "2x2"]).optional(),

  // Số ô được hiện tính từ đầu (1 = chỉ ô đầu, 2 = hai ô đầu...). Ô mới nhất rơi nhẹ từ trên xuống
  // kèm mờ dần hiện ra; các ô chưa tới lượt bị che bằng đúng màu nền nên trông như chưa được vẽ.
  // Bỏ trống (hoặc không có revealLayout) = hiện nguyên ảnh như trước, mọi video cũ không đổi.
  revealIndex: z.number().int().min(1).optional(),
});
