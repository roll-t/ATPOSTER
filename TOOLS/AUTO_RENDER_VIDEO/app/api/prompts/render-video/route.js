...
export async function POST(req) {
  try {
    ...
    // Dựng/ĐỒNG BỘ manifest.json từ segments client gửi lên:
    //   • PNG người que: segments có trường `elements`
    //   • pexels_talk_video: segments là narration, không có `elements`
    //
    // TRƯỚC ĐÂY khối này chỉ chạy khi manifest CHƯA tồn tại (`!fs.existsSync`). Hậu quả: viết lại
    // kịch bản cho cùng một dự án (vd 4 đoạn -> 15 đoạn) thì manifest vẫn kẹt ở bản đầu tiên, và
    // render-project.mjs đọc manifest nên dựng ra video theo kịch bản CŨ — lồng tiếng đủ 15 file
    // audio nhưng video chỉ dài đúng 4 đoạn rồi dừng.
    //
    // Giờ manifest luôn được đồng bộ theo segments mới nhất, nhưng phải GIỮ LẠI các trường chỉ
    // manifest mới có (giọng đọc đã dùng, mốc thời gian từng từ, danh sách file ảnh đã sinh) —
    // client không gửi những trường này, ghi đè thẳng là mất sạch.
    const manifestPath = path.join(targetProjectDir, 'manifest.json');
    if (Array.isArray(segmentsForManifest) && segmentsForManifest.length > 0) {
      ...
      if (fs.existsSync(manifestPath)) {
        try {
          ...
          console.warn('[API RenderVideo] manifest.json cũ hỏng, dựng lại từ đầu:', err.message);
        }
      }
      const previousByNumber = new Map(
        (existingManifest?.segments || []).map(s => [s.segmentNumber, s])
      );

      const segments = segmentsForManifest.map(s => {
        const prev = previousByNumber.get(s.segmentNumber);
        // Lời kể đổi thì mốc thời gian từng từ (karaoke) và file ảnh của đoạn cũ không còn khớp
        // nữa — bỏ đi để bước sau sinh lại, giữ lại là chạy chữ sai nhịp.
        const narrationUnchanged = prev && prev.dialogueOrNarration === s.dialogueOrNarration;
        return {
          ...
          status: 'completed',
          files: narrationUnchanged && Array.isArray(prev.files) ? prev.files : [],
          // Giọng đọc gắn với LỰA CHỌN của người dùng cho đoạn đó, không gắn với nội dung câu chữ
          // -> giữ kể cả khi lời kể đã sửa, để đọc lại không bị lạc giọng so với các đoạn khác.
          ...(prev?.voice ? { voice: prev.voice } : {}),
          ...(narrationUnchanged && Array.isArray(prev.wordTimings) && prev.wordTimings.length > 0
      ...
      const manifest = {
        // Giữ nguyên các khoá khác đã có sẵn trong manifest (vd bgMusic do bước chọn nhạc ghi vào).
        ...(existingManifest || {}),
        title: titleForManifest || existingManifest?.title || projectFolder,
      ...
      // Chỉ ghi khi thực sự có thay đổi — tránh đụng vào mtime của manifest ở mỗi lần render lại.
      const changed = !existingManifest
        || JSON.stringify(existingManifest.segments || []) !== JSON.stringify(segments)
        || existingManifest.title !== manifest.title
        || existingManifest.orientation !== manifest.orientation;

      if (changed) {
        fs.mkdirSync(targetProjectDir, { recursive: true });
        fs.writeFileSync(manifestPath, JSON.stringify(manifest, null, 2), 'utf8');
        console.log(
          `[API RenderVideo] Đồng bộ manifest.json: `
          + `${existingManifest?.segments?.length ?? 0} -> ${segments.length} đoạn `
          + `(${category === 'pexels_talk_video' ? 'pexels-talk' : 'PNG'} mode)`
        );
      }
    }
