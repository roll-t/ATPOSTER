...
export function buildSlideshowRemotionConfig(record, processedInput, bgColor = '#0E0F13') {
  ...
  return {
    ...
    orientation,
    captionPosition: ['moral_talk_slideshow', 'buddhist_wisdom', 'japanese_history'].includes(record.category) ? 'top' : 'bottom',
    imageFit: 'cover',
    kenBurns: true,
    transitionSeconds: 0.5,
    bgColor,
    fontFamily: "'Be Vietnam Pro','Noto Sans',Arial,sans-serif",
    captionMode: ['moral_talk_slideshow', 'buddhist_wisdom', 'japanese_history'].includes(record.category) ? 'full' : 'chunked',
