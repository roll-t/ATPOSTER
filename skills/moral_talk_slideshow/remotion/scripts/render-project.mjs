...
const scenes = manifest.segments.map((seg) => {
  ...
  return {
    ...
    // Chỉ truyền durationSeconds khi KHÔNG có file âm thanh (vd chapter-title không lời đọc),
    // nếu có audio thì để trống để calculateMetadata trong Root.tsx đo chính xác thời lượng giọng đọc,
    // nói hết câu là chuyển cảnh ngay lập tức chứ không chờ thời gian mặc định.
    ...(!hasAudio ? (seg.durationSeconds ? { durationSeconds: Number(seg.durationSeconds) } : (seg.layout === 'chapter-title' ? { durationSeconds: 3 } : {})) : {}),
