        ...
        calculateMetadata={async ({ props }) => {
          ...
          const scenesWithDuration: Scene[] = await Promise.all(
            props.scenes.map(async (scene): Promise<Scene> => {
              // Thử đo thời lượng audio thật trước. Nếu có file audio thì ưu tiên 100% thời lượng của giọng đọc
              // để khi nói hết câu là chuyển ngay sang slide kế tiếp, không bị ngưng trệ hay chờ thời gian mặc định.
              if (scene.audio) {
                try {
                  ...
                  if (audioSeconds > 0) {
                    return {
                      ...
                      durationSeconds: audioSeconds + (props.audioPaddingSeconds ?? 0),
                    } as Scene;
                  }
                } catch {
                  // File audio không đọc được, rơi xuống durationSeconds
                }
              }
